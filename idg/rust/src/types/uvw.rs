use std::{f32::consts::PI, io, ops::Add};
...
impl UvwArrayExtension for UvwArray {
    ...
    fn generate(cli: &Cli) -> Self {
        ...
        let time_samples = Array::from_iter(0..cli.timestep_count()).mapv(|x| x as f32);
        ...
        let max_uv = 0.7 * (cli.grid_size / 2) as f32;
        ...
        for (baseline, ratio) in baseline_ratios.iter().enumerate() {
            ...
            for t in 0..cli.timestep_count() as usize {
                uvw[(baseline, t)] = Uvw::new(
                    u_coords[t] + (cli.grid_size / 2) as f32,
                    v_coords[t] + (cli.grid_size / 2) as f32,
                    0.,
                );
            }
        }
