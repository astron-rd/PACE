...
fn main() -> anyhow::Result<()> {
    ...
    let uvws = idg::types::UvwArray::from_file(&PathBuf::from("input/uvw.npy"))?;
    let metadata = MetadataArray::from_file(&PathBuf::from("input/metadata.npy"))?;
    let frequencies = FrequencyArray::from_file(&PathBuf::from("input/frequencies.npy"))?;
    let visibilities = VisibilityArray::from_file(&PathBuf::from("input/visibilities.npy"))?;

    let hdf5_output = File::create("output.hdf5")?;
    let group = hdf5_output.create_group("input")?;
