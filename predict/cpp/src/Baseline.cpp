...
std::vector<int> NSetupSplitUVW(unsigned int nant, const std::vector<int> &ant1,
  ...
  // station UVWs. They are in such an order that the UVW of a station is
  // known before used in another baseline to derive the UVW of the other
  // station. It can handle cases where baselines occur in disjoint station
  // groups like 0-1, 0-2, 1-2 and 3-4, 4-5, 5-6. Note that the first station
  // of a group gets UVW=0. All other station UVWs are relative to it using
  // the baseline UVWs. Also note that nr of groups can be derived from the
  // size of the returned vector (because it contains no entry for the first
  // antenna in a group).
  std::vector<int> uvw_bl;
  uvw_bl.reserve(nant);
  xt::xtensor<bool, 1> known({nant}, false);
  unsigned int nset = 0;
  // Loop until all stations are set.
  while (nset < nant) {
    // Disjoint groups might exist, so keep a vector containing related
    // antennae which are members of the same group.
...
std::vector<Baseline> MakeBaselines(const std::vector<int> &antenna1,
  ...
  std::vector<Baseline> baselines(antenna1.size());
  for (size_t i = 0; i < antenna1.size(); i++) {
    baselines[i] = std::make_pair(antenna1[i], antenna2[i]);
  }
  return baselines;
}
