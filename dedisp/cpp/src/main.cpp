...
int main() {
  ...
  const float time_resolution =
      down_sampling_ratio * sampling_period; // s (0.25 ms sampling)
  ...
  const float frequency_resolution =
      -1.0 * bandwidth / n_channels; // MHz   (This must be negative!)
  ...
  dedisp::FDDPlan fdd_plan(n_channels, time_resolution, peak_frequency,
