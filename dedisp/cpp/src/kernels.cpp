...
void fourier_domain_dedisperse(size_t dm_count, size_t n_frequencies, size_t n_channels, size_t time_resolution, float *spin_frequencies, float *dispersion_measures, float *delays, size_t stride_in, size_t stride_out, std::complex<float>* input, std::complex<float>* output) {
    for (size_t dm_index = 0; dm_index < dm_count; ++dm_index) {
        ...
        for (size_t channel_index = 0; channel_index < n_channels; ++channel_index) {
            dm_delays[channel_index] = dispersion_measures[dm_index] * delays[channel_index] * time_resolution;
        }

        // Loop over spin frequencies
        for (size_t frequency_index = 0; frequency_index < n_frequencies; ++frequency_index) {
            ...
            for (size_t channel_index = 0; channel_index < n_channels; ++channel_index) {
                // Compute phasor
                float phase = 2.0f * std::numbers::pi_v<float> * spin_frequencies[frequency_index] * dm_delays[channel_index];
